...
export const UploadForm: React.FC<UploadFormProps> = ({ onFilesChange, onProcess, files, mode, setMode, genre, setGenre }) => {
  ...
  const maxFiles = mode === 'compare' ? 3 : 1;
  const canProcess = files.length >= minFiles && files.length <= maxFiles;
          ...
          <div onDragOver={handleDragOver} onDrop={handleDrop}>
            <div className="flex justify-center items-center mb-6">
              <div className="p-4 bg-gray-900/50 rounded-full border-2 border-cyan-400/50 shadow-[0_0_15px_rgba(72,187,255,0.3)]">
                <Icon name={mode === 'compare' ? 'compare' : 'upload'} className="w-10 h-10 text-cyan-300" />
              </div>
            </div>
            <h2 className="text-2xl font-bold text-white mb-2">{mode === 'compare' ? 'Upload 2-3 Levels' : 'Upload Your Level'}</h2>
